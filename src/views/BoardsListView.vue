      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
          ...
          <div class="flex gap-2">
            ...
            <button
              @click="showCreate = false; newTitle = ''"
              class="text-sm text-gray-400 px-3 hover:text-gray-600"
            >
              Cancel
            </button>
          </div>
...
async function fetchBoards() {
  ...
  try {
    ...
    // sesuaikan dengan shape response API kamu
...
async function handleCreate() {
  ...
  creating.value = true
  try {
    const board = await createBoard(newTitle.value.trim())
