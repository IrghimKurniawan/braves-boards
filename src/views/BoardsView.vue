    ...
    <div class="flex gap-3 overflow-x-auto pb-4 px-1">
      ...
      <div v-for="(board, index) in boards" :key="index"
        ...
        <div class="flex-1 overflow-y-auto px-2.5 py-2 flex flex-col gap-2">
          <div v-if="!board.tasks?.length" class="text-xs text-gray-400 text-center py-8">
            No tasks
          </div>
          ...
          <div v-for="(task, i) in (board.tasks ?? [])" :key="i"
            ...
            <div class="flex flex-wrap gap-1.5 mb-2">
              ...
              <span v-if="task.checklist?.length > 0" class="flex items-center gap-1 text-[11px] px-2 py-0.5 rounded"
                :class="task.checklist.filter((c: { done: boolean }) => c.done).length === task.checklist.length
            ...
            <div class="flex items-center justify-between mt-1">
              ...
              <div class="flex items-center gap-1 text-[11px] tabular-nums"
                ...
                {{ task.time || '00:00:00' }}
                <button class="ml-1 text-[10px] px-1.5 py-0.5 rounded border transition" :class="activeTimerTaskId === task.id
                  ? 'border-red-200 text-red-500 bg-red-50 hover:bg-red-100'
                  : 'border-gray-200 text-gray-400 hover:bg-gray-100'" @click.stop="handleTimerToggle(task)">
              ...
              <div class="relative">
                ...
                <div v-if="statusOpen"
                  ...
                  <button v-for="s in statuses" :key="s" @click.stop="selectedTask.status = s; statusOpen = false"
                    ...
                    :class="selectedTask.status === s ? 'text-blue-600 font-semibold' : 'text-gray-700'">{{ s
                    }}</button>
                </div>
              </div>
                ...
                <div class="flex flex-wrap gap-2 mb-6">
                  <div class="relative">
                    ...
                    <div v-if="addMenuOpen"
                      ...
                      <div class="py-1">
                        <button v-for="item in addMenuItems" :key="item.action"
                    ...
                    <div v-for="(att, i) in selectedTask.attachments" :key="i"
                      ...
                      <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                          ...
                          <p class="text-xs text-gray-400 mt-0.5">{{ att.type === 'image' ? 'Image' : att.type ===
                            'link'
                            ? 'Link' :
                            'File' }}</p>
                        </div>
